...
Hard ATS hosts such as Workday, iCIMS, Taleo, Oracle Recruiting, and Avature can now fall back to a Codex-assisted apply planner. That path lets Codex inspect the live page state between steps and suggest the next browser actions, which is much closer to the original Claude-driven behavior on tricky forms.
